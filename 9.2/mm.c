...
#define PACK(size, alloc, alloc_prev)  (((size) | (alloc)) | (alloc_prev << 1)) //line:vm:mm:pack
...
void *mm_malloc(size_t size)
{
    ...
    // Since no longer need footers, Dsize does not need to be multiplied by 2
...
void mm_free(void *bp)
{
    ...
    PUT(HDRP(bp), PACK(size, 0, GET_ALLOC(HDRP(PREV_BLKP(bp)))));
    PUT(FTRP(bp), PACK(size, 0, GET_ALLOC(HDRP(PREV_BLKP(bp)))));
...
static void *coalesce(void *bp)
{
    ...
    if (alloc_prev && next_alloc) {            /* Case 1 */
        return bp;
    }

    else if (alloc_prev && !next_alloc) {      /* Case 2 */
        size += GET_SIZE(HDRP(NEXT_BLKP(bp)));
        PUT(HDRP(bp), PACK(size, 0, 1));
        PUT(FTRP(bp), PACK(size,0, 1));
    }

    else if (!alloc_prev && next_alloc) {      /* Case 3 */
...
static void *extend_heap(size_t words)
{
    ...
    PUT(HDRP(bp), PACK(size, 0, GET_ALLOC(HDRP(PREV_BLKP(bp)))));         /* Free block header */   //line:vm:mm:freeblockhdr
    PUT(FTRP(bp), PACK(size, 0, GET_ALLOC(HDRP(PREV_BLKP(bp)))));         /* Free block footer */   //line:vm:mm:freeblockftr
